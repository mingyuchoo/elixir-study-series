defmodule PlayaWeb.UserLive.Settings do
      ...
      <div>
        ...
        >
          <.input
            ...
            type="text"
            label="Nickname"
            placeholder="Set your awesome nickname"
          />
          <:actions>
  ...
  def mount(_params, _session, socket) do
    ...
    nickname_changeset = Accounts.change_user_nickname(user)
  ...
  def handle_event("validate_nickname", %{"user" => user_params}, socket) do
    ...
    new_socket =
      socket
      |> assign(:nickname_form, to_form(changeset))
  ...
  def handle_event("update_nickname", %{"user" => user_params}, socket) do
    socket.assigns.current_user
    |> Accounts.update_user_nickname(user_params)
    |> case do
      {:ok, _user} ->
        {:noreply, socket |> put_flash(:info, "User updated successfully")}

      {:error, changeset} ->
        {:noreply, assign(socket, :nickname_form, to_form(changeset))}
    end
  end
